...
class buildTrainValtest():
    ...
    def getNonRepetitiveData(self,n_start, type='train'):
        ...
        maplist = {'Present': 0, 'Absent': 1, 'Weak': 2, 'Uncertain': 3}
    ...
    def functionCheck(self):
        presentCounts = len(self.trainPieces['Present']) + len(self.testPieces['Present'])
        absentCounts = len(self.trainPieces['Absent']) + len(self.testPieces['Absent'])
        weakCounts = len(self.trainPieces['Weak']) + len(self.testPieces['Weak'])
        uncertainCounts = len(self.trainPieces['Uncertain']) + len(self.testPieces['Uncertain'])
        print('present:',presentCounts,'\nabsent:',absentCounts,'\nweak:',weakCounts,'\nuncertain:',uncertainCounts)
